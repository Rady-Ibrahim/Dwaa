...
@section('content')
    ...
    <div class="mb-6 rounded-2xl border p-5 flex items-center justify-between gap-4"
        ...
        <div class="flex items-center gap-3">
            {{-- تعديل الحد الأقصى --}}
            <div class="flex items-center gap-2 rounded-xl border px-4 py-2"
                style="border-color: var(--border-subtle); background: var(--surface-raised);">
                <label for="maxDevicesInput" class="text-xs text-zinc-500 whitespace-nowrap">الحد الأقصى</label>
                <input type="number" id="maxDevicesInput" min="1" max="50" value="{{ $max_devices }}"
                    class="w-16 rounded-lg border px-2 py-1 text-center text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-rose-500/50"
                    style="border-color: var(--border-subtle); background: var(--surface-card);">
                <button id="saveMaxBtn" type="button"
                    class="inline-flex items-center gap-1 rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-rose-500 transition disabled:opacity-40">
                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                    حفظ
                </button>
                <svg id="maxDevicesSpinner" class="hidden h-4 w-4 animate-spin text-zinc-400" fill="none"
                    viewBox="0 0 24 24">
            ...
            <div class="rounded-xl border px-4 py-2 text-center min-w-[100px]"
                style="border-color: var(--border-subtle); background: var(--surface-raised);">
                <p
                    class="text-2xl font-bold {{ $devices->count() >= $max_devices ? 'text-rose-400' : 'text-emerald-400' }}">
    ...
    @if ($devices->count() >= $max_devices)
        <div
            class="mb-5 rounded-2xl border border-amber-500/30 bg-amber-950/20 px-4 py-3 text-sm text-amber-300 flex items-center gap-3">
            <svg class="h-5 w-5 shrink-0 text-amber-400" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
            </svg>
            <span>وصل المستخدم للحد الأقصى ({{ $max_devices }} أجهزة). لن يتمكن من تسجيل الدخول من أي جهاز جديد حتى تحذف
                أحد الأجهزة.</span>
        </div>
    @endif
    ...
    <div class="overflow-x-auto rounded-xl border" style="border-color: var(--border-subtle);" id="devicesTable">
        @if ($devices->isEmpty())
            <div class="p-16 text-center text-zinc-500">
                <svg class="mx-auto h-12 w-12 mb-4 opacity-20" fill="none" stroke="currentColor" stroke-width="1.5"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @else
            <table class="min-w-full divide-y text-sm" style="border-color: var(--border-subtle);">
                <thead class="text-right text-xs uppercase text-zinc-500" style="background: var(--surface-raised);">
                    <tr>
                    ...
                    @foreach ($devices as $i => $device)
                        <tr class="transition hover:bg-white/[0.02]" id="device-row-{{ $device->id }}">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"
                                        ...
                                        @if (str_contains(strtolower($device->device_name ?? ''), 'ios') ||
                                                str_contains(strtolower($device->device_name ?? ''), 'iphone') ||
                                                str_contains(strtolower($device->device_name ?? ''), 'ipad'))
                                            <svg class="h-5 w-5 text-zinc-300" fill="currentColor" viewBox="0 0 24 24">
                                                <path
                                        ...
                                        @elseif(str_contains(strtolower($device->device_name ?? ''), 'android'))
                                            <svg class="h-5 w-5 text-emerald-400" fill="currentColor"
                                                viewBox="0 0 24 24">
                                                <path
                                                    d="M17.523 15.341c-.3 0-.546-.246-.546-.546V9.205c0-.301.246-.546.546-.546.3 0 .546.245.546.546v5.59c0 .3-.246.546-.546.546zm-11.046 0c-.3 0-.546-.246-.546-.546V9.205c0-.301.246-.546.546-.546.3 0 .546.245.546.546v5.59c0 .3-.246.546-.546.546zm1.909-9.728L7.24 4.31a.268.268 0 01.381-.38l1.192 1.192A5.6 5.6 0 0112 4.547c.77 0 1.5.155 2.167.427L15.38 3.93a.269.269 0 01.381.381l-1.146 1.147A5.515 5.515 0 0117.09 8.5H6.91a5.515 5.515 0 012.476-2.887zM10.182 7a.545.545 0 110-1.09.545.545 0 010 1.09zm3.636 0a.545.545 0 110-1.09.545.545 0 010 1.09zM6.477 9.59h11.046v6.682a1.09 1.09 0 01-1.09 1.09h-.546v2.092a.818.818 0 11-1.637 0v-2.091H9.75v2.091a.818.818 0 11-1.636 0v-2.091h-.546a1.09 1.09 0 01-1.09-1.09V9.59z" />
                                            </svg>
                                    ...
                                    <span
                                        class="font-medium text-white">{{ $device->device_name ?: 'جهاز غير معروف' }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-center">
                                <button onclick="deleteDevice({{ $user->id }}, {{ $device->id }})"
    ...
    <div class="mt-6 rounded-2xl border px-5 py-4 text-sm text-zinc-500"
        ...
        <ul class="space-y-1 list-disc list-inside">
            <li>يمكن تعديل الحد الأقصى للأجهزة من حقل <strong class="text-zinc-300">الحد الأقصى</strong> أعلاه.</li>
...
@push('scripts')
    <script>
        ...
        const maxDevicesUrl = deviceBaseUrl + '/max';
        ...
        var maxDevicesInput = document.getElementById('maxDevicesInput');
        var saveMaxBtn = document.getElementById('saveMaxBtn');
        var maxDevicesSpinner = document.getElementById('maxDevicesSpinner');

        if (saveMaxBtn && maxDevicesInput) {
            saveMaxBtn.addEventListener('click', function() {
                var val = parseInt(maxDevicesInput.value);
                if (isNaN(val) || val < 1 || val > 50) {
                    showToast('أدخل رقم صحيح (1 - 50)', 'error');
                    return;
                }

                saveMaxBtn.disabled = true;
                maxDevicesSpinner.classList.remove('hidden');
                ...
                fetch(maxDevicesUrl, {
                    ...
                    .then(function(data) {
                        maxDevicesSpinner.classList.add('hidden');
                        saveMaxBtn.disabled = false;
                        maxDevicesInput.value = data.max_devices;
                        updateMax(data.max_devices);
        ...
        function updateMax(newMax) {
            ...
            counter.textContent = current + ' / ' + newMax;
            counter.classList.remove('text-rose-400', 'text-emerald-400');
            counter.classList.add(current >= newMax ? 'text-rose-400' : 'text-emerald-400');
        }
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', function() {
                ...
                apiFetch(deviceBaseUrl)
                    .then(function(res) {
                        document.querySelectorAll('[id^="device-row-"]').forEach(function(r) {
                            r.remove();
                        });
                        ...
                        if (tbody) {
                            ...
                            if (tableDiv) {
                                tableDiv.innerHTML =
                                    '<div class="p-16 text-center text-zinc-500"><p class="text-sm">تم مسح جميع الأجهزة المسجلة</p></div>';
                            }
                        }
        ...
        function showToast(message, type) {
            ...
            toast.style.cssText = 'position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);background:' + bg +
                ';border:1px solid ' + border + ';color:' + color +
                ';padding:0.75rem 1.5rem;border-radius:12px;font-size:0.875rem;z-index:9999;box-shadow:0 10px 25px rgba(0,0,0,0.5);white-space:nowrap;';
